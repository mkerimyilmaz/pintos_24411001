...
/* Maximum number of syscall functions. */
typedef int pid_t;
static int (*syscall_handlers[20]) (struct intr_frame *); /* Array of syscall functions */

/* Reads a byte at user virtual address UADDR.
   UADDR must be below PHYS_BASE.
   Returns the byte value if successful, or -1 if a segfault occurred. */
static int
...
static bool
is_valid_pointer (void *esp, size_t count)
{
  size_t i;
  for (i = 0; i < count; i++)
  {
    if (get_user_byte ((uint8_t *)esp + i) == -1)
      return false;
  }
  return true;
}

/* Check whether a given user string is valid. */
static bool
is_valid_string (void *str)
{
  int ch = -1;
  while ((ch = get_user_byte((uint8_t *)str++)) != '\0' && ch != -1)
    ;
  return ch == '\0';
}
...
syscall_exec_wrapper (struct intr_frame *f)
{
  ...
  if (strlen(str) >= PGSIZE) {
    printf("Strings longer than one page are not supported\n");
    return -1;
  }
  if (strlen(str) == 0 || str[0] == ' ') {
    printf("The command string must be non-empty and not begin with a space: %s\n", str);
    return -1;
  }
