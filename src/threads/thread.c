...
thread_tick (void)
{
  ...
  if (thread_mlfqs)
    {
      t->last_cpu_retrieved = fixed_add_int (t->last_cpu_retrieved, 1);
...
{//todo
  return fixed_to_int (fixed_mul (thread_current ()->last_cpu_retrieved, 100));
}

...
void process_tick_event(void)
{
  ...
  load_avg = fixed_add (fixed_div (fixed_mul (load_avg, 59), 60),
  fixed_div (int_to_fixed (waiting_threads), 60));
...
{ 
  t->last_cpu_retrieved = fixed_add_int (fixed_div (fixed_mul_int (fixed_mul (load_avg, 2), t->last_cpu_retrieved),
  fixed_add_int (fixed_mul (load_avg, 2), 1)), t->nice);
  recalculate_mlfqs_priority (t);
}
void recalculate_mlfqs_priority(struct thread *t)
{
  int new_priority = (int) fixed_to_int (fixed_sub (int_to_fixed ((PRI_MAX - ((t->nice) * 2))),
  fixed_div (t->last_cpu_retrieved, 4)));
